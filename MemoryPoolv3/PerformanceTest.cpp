...
public:
    ...
    static void warmUp() {
        ...
        for(int i = 0; i < 1000; ++i) {
            for(size_t size : {8, 16, 32, 64, 128, 256, 512, 1024}) {
                ...
                // 存储指针和对应的大小
                warmupPtrs.emplace_back(ptr, size);
            }
        }
    ...
    static void testSmallAllocation() {
        // NUM_ALLOCS：定义了总共进行 10,000 次 内存分配。
        constexpr size_t NUM_ALLOCS = 10000;
        // 使用固定的几个小对象大小，这些大小都是内存池最优化的大小
        const size_t SIZES[] = {8, 16, 32, 64, 128, 256};
        // NUM_SIZES：根据 SIZES 数组的大小，计算出需要测试的不同内存块大小的数量。它的值是 6，表示我们将进行 6 种不同大小的内存分配。
        ...
        // 测试内存池
        {
            ...
            for(auto& ptrs: sizePtrs) {
                // 使用 ptrs.reserve(NUM_ALLOCS / NUM_SIZES) 为每个大小类的 vector 预留内存，确保其可以容纳所有的分配。
                ptrs.reserve(NUM_ALLOCS / NUM_SIZES);
            }

            for(size_t i = 0; i < NUM_ALLOCS; ++i) {
                // 循环使用不同大小
                // i % NUM_SIZES 保证每次分配的大小是从 6 个大小值中循环选择的
        ...
        // 测试new/delete
        {
            ...
            for(size_t i = 0; i < NUM_ALLOCS; ++i) {
                ...
                if(i % 4 == 0) {
                    ...
                    if(!ptrs.empty()) {
                        // 由于使用了 new[] 分配了一个 char[] 类型的数组，所以在释放时需要将 void* 转换为 char*，以确保 delete[] 正确释放内存。
                        // static_cast<char*>(ptrs.back().first) 将 void* 类型的指针转换为 char* 类型，以适应 delete[] 操作
